...
- [x] **B3.** Migration 0014 — `transfer_workspace_ownership` RPC + pg-tap `test/sql/test_migration_0014.sql` (17 assertions, all passing on lane dev). (actual: ~3 hours vs ~2 hour estimate; overrun absorbed pre-B3 spec patch sweep [14 sites including fatal `owner_user_id` → `owner_id` at §4.3 line 435], drizzle-kit silent no-op for function-only migrations [3-artifact manual scaffold required], and audit_log INSERT compatibility probe). Shipped across 4 commits: 58db6b7 (docs spec patch pre-B3) → ad8ff09 (feat 0014 RPC migration) → 5cc94d4 (test pg-tap) → docs sync. Applied to lane dev only; lane app apply deferred to separate production session.
...
*Last updated: April 23, 2026 — Week 7.5a Phase A complete (A1, A2a, A2b, A3) + Phase B steps B1, B2, and B3 shipped (migrations 0011 + 0012 + 0013 + 0014). Next: B4 (migration 0015 — `profiles.left_at` + orphaned-workspace admin view + pg-tap, ~1 hour estimate).*
...
- ~~[2026-04-20] Spec column-name drift: docs/user-flows-spec.md §4.4 references organizations.owner_user_id; actual schema column is organizations.owner_id (db/schema/users.ts:6).~~ **Resolved 2026-04-23 in commit `58db6b7`** — option (i) "patch spec" chosen over column rename (smaller blast radius). Mechanical sweep grepped `owner_user_id` across spec; 14 sites patched (§4.3 line 435 fatal usage `UPDATE organizations SET owner_user_id = ...`, plus 13 other §4.3/§4.4 references). Verified zero remaining `owner_user_id` references in spec post-commit. Migration 0014 implementation uses `SET owner_id = new_owner_user_id` (column-qualified, matches schema).
...
- ~~[2026-04-22] B3 design — transfer_workspace_ownership 2-col vs 3-col return.~~ **Resolved 2026-04-23 in commit `58db6b7`** — spec locked to 2-col `RETURNS TABLE (success boolean, message text)` (line 474 patched). Implementation in commit `ad8ff09` matches: `RETURNS TABLE (success boolean, message text)`. Pg-tap assertion #2 (signature check) verifies the 2-col signature on lane dev. Decision rationale: simpler caller contract, no ambiguity about which column carries the user-visible message, error messages embedded in `message` column on `success=false` returns.
...
- ~~[2026-04-23] Fix-up migration snapshot discipline — when a fix-up migration ships with no schema changes (e.g., 0012_fix_rpc_org_id_ambiguity), `drizzle-kit generate` is easy to skip since there's nothing to diff. Authoring the snapshot by copying the previous one (byte-identical content, semantically correct) fails to regenerate the `id` and `prevId` metadata, creating a latent chain collision that surfaces at the next real generate. Surfaced B2 PART 2; repaired in commit 2936339.~~ **Resolved 2026-04-23 (this commit, session-end docs sync)** — subsumed by the new "Function-only migration scaffold pattern" rule in WORKING-RULES.md, which makes id/prevId regeneration mandatory for any migration without schema changes (covers both fix-ups and feature migrations like 0014 B3). The WORKING-RULES section explicitly calls out the "reusing previous snapshot's id and prevId verbatim creates a latent chain collision" failure mode, with the same root cause as B2 PART 2.
...
- ~~[2026-04-23] Spec references to migration filenames drift on renumber — no existing trigger catches this. Example: `user-flows-spec.md` §4.2 heading still says `0012_invite_team_scoping.sql` despite the post-B1 renumber to 0013 (missed in cc8e18f).~~ **Resolved 2026-04-23 in commit `58db6b7`** — pre-B3 spec patch sweep ran the mechanical audit (`grep -E '0[0-9]{3}_\w+\.sql'` against spec) and patched all stale filename references. Migration 0014's spec section (§4.3) correctly references `0014_b3_ownership_transfer.sql`. Mechanical-audit recommendation now codified as recurring discipline: run the grep after every migration renumber. Adding the recurrence rule to the Discipline section below as a separate parking-lot entry.
...
*From B3 session (April 23):*
- ~~[2026-04-23] Function-only migration scaffold pattern — drizzle-kit generate produces no output for migrations that add functions but no schema changes. Surfaced reactively at 0012 fix-up (B1 RPC ambiguity, commit 1724920) and proactively at 0014 B3 (commit ad8ff09). 3 artifacts must be hand-created: (1) the SQL migration file, (2) `meta/NNNN_snapshot.json` copied byte-identical from previous snapshot with regenerated `id` UUID and `prevId` pointing at previous snapshot's id, (3) `meta/_journal.json` idx=N entry appended with current wall-clock `when` (verify `when > MAX(when)` across existing entries to satisfy strict-greater invariant). The previous-occurrence parking-lot entry (Line 515 fix-up snapshot discipline) called this out reactively for fix-ups; B3 confirms it applies to any function-only migration. 2nd occurrence; codifying ahead of 3rd.~~ **Resolved 2026-04-23 (this commit, session-end docs sync)** — codified in WORKING-RULES.md "Function-only migration scaffold pattern" section. Pattern now mandatory for any migration that adds/modifies SQL functions without column changes. Subsumes Line 515 fix-up snapshot discipline parking-lot entry.
- [2026-04-23] Spec drift mechanical sweep — codify the grep audit ran during pre-B3 (commit 58db6b7) as a recurring discipline. Rule shape: after every migration renumber, after every spec edit that mentions a SQL identifier (column, table, type, function), and as a quarterly backstop, run mechanical greps over `docs/user-flows-spec.md` for: (a) `\d{4}_\w+\.sql` filename references against `db/migrations/` directory listing; (b) bare SQL identifiers cross-checked against `information_schema` (catches column rename drift, e.g., owner_user_id → owner_id). Pre-B3 sweep caught 14 sites including 1 fatal usage. Without the audit, B3 composition would have failed at first-statement execution. Trigger to promote: third independent occurrence of spec-drift-as-blocker (counting Line 461 owner_user_id and L517 filename refs as the first two). Lean toward WORKING-RULES codification at next occurrence. Budget when triggered: 15-30 min to draft rule + add to WORKING-RULES + update review cadence.
...
**Active items:** 72 — update whenever parking lot entries are added or resolved. 2026-04-23 (B3 session): 75 → 72 — 4 resolutions (Lines 461 owner_user_id spec drift, 474 B3 2-col vs 3-col return, 515 fix-up snapshot discipline subsumed by new WORKING-RULES rule, 517 spec filename-ref drift) + 1 self-resolving entry (function-only migration scaffold pattern, codified in WORKING-RULES this commit) + 1 net new entry (spec drift mechanical sweep recurring discipline candidate). 2026-04-23 (B2 session): 73 → 75 — 3 resolutions (Lines 473, 507, 508) + 5 new entries (projects.ts:46 orphan write, fix-up snapshot discipline, existing_profile unused var, spec filename-reference drift, probe-script convention) + 1 reclassification of Line 461 (still active, trigger updated). Previous count of 12 was stale; corrected 2026-04-20 during pre-B1 bootstrap commit.
