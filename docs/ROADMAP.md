...
> **Next session:** Item 1 is done and ticked. Item 4 (Intake check UI, 5 hours) is next in Week 1 sequence. Plan: build Item 4 in local dev this session; fix production issues (ANTHROPIC_API_KEY 401 in Vercel, database pool exhaustion) in a follow-up session before deploying Item 4 to users. Read `docs/WORKING-RULES.md` at the start of every new chat.
