...
-- Separate INSERT/UPDATE/DELETE policies so SELECT is not conflated with mutation grants
CREATE POLICY "weekly_digests_privileged_insert"
ON public.weekly_digests
FOR INSERT
TO public
WITH CHECK (
...
CREATE POLICY "weekly_digests_privileged_update"
ON public.weekly_digests
FOR UPDATE
TO public
USING (
...
CREATE POLICY "weekly_digests_privileged_delete"
ON public.weekly_digests
FOR DELETE
TO public
USING (
...
-- INSERT is system-only (alerts are generated server-side by cron/lead actions).
-- When those paths migrate off systemDb, this policy must be revisited.
CREATE POLICY "proactive_alerts_system_insert"
...
-- INSERT is system-only (generated by cron). When the cron route migrates off
-- systemDb, this policy should be tightened to a service role.
CREATE POLICY "morning_briefings_system_insert"
